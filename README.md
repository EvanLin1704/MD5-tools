...
`.github/workflows/build.yml` 會在三個官方 runner 上同時打包 Windows / Linux / macOS。

觸發方式：

- **手動**：到 GitHub 的 **Actions** 分頁選 `build` → **Run workflow**。
- **推送 tag**：推送符合 `v*` 的標籤時自動執行。

```bash
# 發布新版本前，先把 package.json 的 version 改好（畫面顯示的版本也會跟著變）
git tag v1.0.0
git push origin v1.0.0
```

跑完後到該次 run 最下方的 **Artifacts** 下載三個平台的檔案。
工作流程只使用官方 runner 與第一方 action，且 macOS 不簽章，皆在 GitHub 免費額度內。
